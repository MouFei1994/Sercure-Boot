...
It also demonstrate key management services through basic examples and specific boot
...
@par Directory contents
   ...
   - 2_Images_UserApp/Src/kms_object_update_app.c  KMS Blob update application
   ...
   - 2_Images_UserApp/Src/tkms_app.c               tKMS tests
   - 2_Images_UserApp/Src/tkms_app_derive_key.c    tKMS key derivation tests
   - 2_Images_UserApp/Src/tkms_app_encrypt_decrypt.c tKMS encryption/decryption tests
   - 2_Images_UserApp/Src/tkms_app_sign_verify.c   tKMS signature/verification tests
   ...
   - 2_Images_UserApp/Inc/tkms_app.h               Header for tkms_app.c file
...
This postbuild script:
 ...
 - prepares the new portion of firmware image: part of firmware image which changed vs a reference firmware image.
   ...
   the reference binary file and a PartialUserApp.sfb will be generated in 2_Images_UserApp/Binary folder.
...
Once executed, this user application gives access to a menu which allows:
  ...
  4 - to demonstrate usage of key management services
...
2. Pressing 2 allows to test protections.
 ...
 - Firewall tests (#1, #2): cause a reset trying to access protected code or data (either in RAM or FLASH)
 - PCROP test (#3): causes an error trying to access the PCROP region protecting the keys. If activated, 
   Firewall will trigger a reset prior to this error
 - WRP test (#4): causes an error trying to erase write protected code
 - IWDG test (#5): causes a reset simulating a deadlock by not refreshing the watchdog
 - TAMPER test (#6): causes a reset if a tamper event is detected. In order to generate a tamper event,
   user has to connect PA0 (CN3.2) to GND (It may be enough to put your finger close to PA0 (CN3.2)).
   Several resets may be generated during this test.
 - CORRUPT IMAGE test (#7): causes a signature verification failure at next boot
...
4. Pressing 4 allows to enter specific key management services test menu.
