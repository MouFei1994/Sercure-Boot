...
#define TEST_PROTECTIONS_SE_ISOLATED_CODE_FLASH_ADDRESS     ((uint32_t) SE_CODE_REGION_ROM_START)
...
/** @defgroup  TEST_PROTECTIONS_Private_Functions Private Functions
  * @{
  */
static void TEST_PROTECTIONS_Run_FWALL_CODE(void);
static void TEST_PROTECTIONS_Run_FWALL_VDATA(void);
static void TEST_PROTECTIONS_Run_PCROP(void);
static void TEST_PROTECTIONS_Run_WRP(void);
...
void TEST_PROTECTIONS_Run(void)
{
  ...
  while (exit == 0U)
  {
    ...
    if (COM_Receive(&key, 1U, RX_TIMEOUT) == HAL_OK)
    {
      ...
      {
        case '1' :
          TEST_PROTECTIONS_Run_FWALL_CODE();
          break;
        case '2' :
          TEST_PROTECTIONS_Run_FWALL_VDATA();
          break;
        case '3' :
          TEST_PROTECTIONS_Run_PCROP();
          break;
        case '4' :
          TEST_PROTECTIONS_Run_WRP();
          break;
        case '5' :
...
{
  printf("\r\n=================== Test Menu ============================\r\n\n");
  printf("  Test Protection: Firewall - CODE ---------------------- 1\r\n\n");
  printf("  Test Protection: Firewall - VDATA --------------------- 2\r\n\n");
  printf("  Test Protection: PCROP -------------------------------- 3\r\n\n");
  printf("  Test Protection: WRP ---------------------------------- 4\r\n\n");
  printf("  Test Protection: IWDG --------------------------------- 5\r\n\n");
  printf("  Test Protection: TAMPER ------------------------------- 6\r\n\n");
  printf("  Test : CORRUPT ACTIVE IMAGE --------------------------- 7\r\n\n");
...
static void TEST_PROTECTIONS_Run_WRP(void)
{
  ...
  else
  {
    ...
    {
      printf("  -- Written succesfully at address: 0x%x\r\n\n", TEST_PROTECTIONS_WRP_FLASH_ADDRESS);
...
static void TEST_PROTECTIONS_Run_CORRUPT_IMAGE(void)
{
  ...
  /* Erase first sector of SLOT 0 */
  printf("  -- Erasing 0x%x bytes at address: 0x%x\r\n", TEST_PROTECTIONS_CORRUPT_IMAGE_FLASH_SIZE,
         TEST_PROTECTIONS_CORRUPT_IMAGE_FLASH_ADDRESS);
  printf("  -- At next boot Signature Verification will fail. Download a new FW to restore UserApp !!\r\n\n");
  ...
  ret = FLASH_If_Write((void *)(TEST_PROTECTIONS_CORRUPT_IMAGE_FLASH_ADDRESS), (void *) &pattern,
                       TEST_PROTECTIONS_CORRUPT_IMAGE_FLASH_SIZE);
...
static void TEST_PROTECTIONS_Run_TAMPER(void)
{
  ...
  /* Print instructions*/
  printf("  -- Pull PA0 (CN3.2) to GND \r\n\n");
  printf("  -- -- Note: sometimes it may be enough to put your finger close to PA0 (CN3.2)\r\n\n");
