...
set "command=%python%%prepareimage% inject -k %ecckey% -f %1\kms_platf_objects_config.h -p @ECDSA_SBSFU_KEY@ %1\kms_platf_objects_config.h.tmp"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
IF [%oemkey%] == [] goto :setdefaultoemkey
goto :next_step
:setdefaultoemkey
set "oemkey=%1\\..\\Binary\\OEM_KEY_COMPANY1_key_AES_CBC.bin"
:next_step
set "command=%python%%prepareimage% inject -k %oemkey% -f %1\kms_platf_objects_config.h.tmp -p @AES_SBSFU_KEY@ %1\kms_platf_objects_config.h"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
