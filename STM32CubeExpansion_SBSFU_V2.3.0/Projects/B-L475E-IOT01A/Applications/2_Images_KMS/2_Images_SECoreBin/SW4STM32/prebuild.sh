...
#no error proceed with next steps
  ...
  #Patch KMS embedded keys with SBSFU Keys
  if [ $ret == 0 ]; then
    if [ -z "$oemkey" ]; then oemkey=$1"/../Binary/OEM_KEY_COMPANY1_key_AES_CBC.bin"; fi
    command=$cmd" "$prepareimage" inject -k "$oemkey" -f "$1"/kms_platf_objects_config.h -p @AES_SBSFU_KEY@ "$1"/kms_platf_objects_config.h.tmp"
    echo $command
    $command
    ret=$?
    if [ $ret == 0 ]; then
      command=$cmd" "$prepareimage" inject -k "$ecckey" -f "$1"/kms_platf_objects_config.h.tmp -p @ECDSA_SBSFU_KEY@ "$1"/kms_platf_objects_config.h"
      echo $command
      $command
      ret=$?
