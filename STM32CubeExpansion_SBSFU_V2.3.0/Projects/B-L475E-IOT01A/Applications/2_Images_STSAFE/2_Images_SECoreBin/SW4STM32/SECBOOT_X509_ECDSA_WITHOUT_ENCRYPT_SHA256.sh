...
fw_sign_ecckey=$projectdir"/../../2_Images_SBSFU/Certs/sbsfu_fs_prv.pem"
cert_fw_leaf=$projectdir"/../../2_Images_SBSFU/Certs/sbsfu_fs_crt.der"
cert_fw_inter=$projectdir"/../../2_Images_SBSFU/Certs/sbsfu_i2_crt.der"
    ...
    if [ $ret == 0 ]; then
      ...
      if [ $ret == 0 ] && [ -e "$ref_userapp" ]; then
        ...
        if [ $ret == 0 ]; then
          ...
          ret=$?
          if [ $ret == 0 ]; then
            command=$cmd" "$prepareimage" pack -k "$fw_sign_ecckey" -p 1 -r 44 -v "$version"  --cert_fw_inter "$cert_fw_inter" --cert_fw_leaf "$cert_fw_leaf" -f "$bin" -t "$sign" -o 2048 --pfw "$partialbin" --ptag "$partialsign" --poffset  "$partialoffset" "$partialsfb
            $command >> $projectdir"/output.txt"
            ret=$?
          fi
        fi
