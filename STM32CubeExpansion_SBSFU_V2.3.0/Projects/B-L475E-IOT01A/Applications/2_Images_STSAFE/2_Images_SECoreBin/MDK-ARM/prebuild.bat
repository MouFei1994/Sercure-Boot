...
::STSAFE Pairing Keys binary is stored in STSAFE_Provisioning project
set "oemkey=%1\\..\\..\\STSAFE_Provisioning\\Binary\\STSAFE_PAIRING_keys.bin"
set "command=%python%%prepareimage% trans -a ARM -k %oemkey% -f SE_ReadKey_Pairing -s .SE_Key_Data -e 1 -v V7M > %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
goto end
