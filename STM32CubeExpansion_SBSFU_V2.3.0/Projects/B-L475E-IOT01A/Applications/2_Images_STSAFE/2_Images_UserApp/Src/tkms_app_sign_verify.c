 ...
 *                                      KMS_KEY_AES_RSA2048_OBJECT_HANDLE object handle
...
CK_RV tkms_app_sign_verify_rsa(uint32_t length, uint8_t *pMessage)
{
  ...
  if (rv == CKR_OK)
  {
    rv = C_SignInit(session, &rsa_mechanism, (CK_OBJECT_HANDLE)KMS_KEY_AES_RSA2048_OBJECT_HANDLE);
  }

  /* Sign message */
  if (rv == CKR_OK)
  {
    rv = C_Sign(session, (CK_BYTE_PTR)pMessage, length,
                signature, (CK_ULONG_PTR)&signature_length);
  }
  ...
  if (rv == CKR_OK)
  {
    rv = C_VerifyInit(session, &rsa_mechanism, (CK_OBJECT_HANDLE)KMS_KEY_AES_RSA2048_OBJECT_HANDLE);
  }

  /* Verify message */
  if (rv == CKR_OK)
  {
    rv = C_Verify(session, (CK_BYTE_PTR)pMessage, length,
  ...
  {
    printf("XXX Message not authenticated\r\n");
  }
...
CK_RV tkms_app_ext_token_sign_verify_ecdsa(void)
{
  ...
  CK_ULONG ul_count, signature_length;
  ...
  if (rv == CKR_OK)
  {
    ...
    ecdsa_mechanism.ulParameterLen = 0;
  ...
  if (rv == CKR_OK)
  {
    ...
    rv = C_Sign(session, (CK_BYTE_PTR)a_Hash, sizeof(a_Hash),
                signature, (CK_ULONG_PTR)&signature_length);
  }
  ...
  printf("--- Device Unique Public Key ----------------------------------------------\r\n");
  ...
  if (rv == CKR_OK)
  {
    rv = C_Verify(session, (CK_BYTE_PTR)a_Hash, sizeof(a_Hash),
                  signature, signature_length);
  }
