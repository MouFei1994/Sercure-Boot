...
if [ $ret == "0" ]; then
    ...
    #AES128_GCM_AES128_GCM_AES128_GCM
        ...
        command=$cmd" "$prepareimage" trans -a GNU -k "$oemkey" -f SE_ReadKey -s .SE_Key_Data -e 1 -v V6M"
        echo $command
        $command > $asmfile
        ret=$?
    ...
    if [ $crypto == "SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256" ]; then  
        ...
        command=$cmd" "$prepareimage" trans  -a GNU -k "$oemkey" -f SE_ReadKey -s .SE_Key_Data -v V6M"
        echo $command
        $command > $asmfile
        ret=$?
        if [ $ret == 0 ]; then
            command=$cmd" "$prepareimage" trans  -a GNU -k "$ecckey" -f SE_ReadKey_Pub -e 1 -v V6M"
            echo $command
            $command >> $asmfile
            ret=$?
        fi
    ...
    #ECCDSA_WITHOUT_ENCRYPT_SHA256
        oemkey=""
        command=$cmd" "$prepareimage" trans  -a GNU -k "$ecckey" -f SE_ReadKey_Pub  -s .SE_Key_Data -e 1 -v V6M"
        echo $command
        $command > $asmfile
        ret=$?
    fi
