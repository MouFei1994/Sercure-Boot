...
SFU_ErrorStatus SFU_LL_SECU_CheckProtectionWRP(FLASH_OBProgramInitTypeDef *psFlashOptionBytes)
{
  ...
  {
    if ((psFlashOptionBytes->WRPSector & (OB_WRP_Pages0to31 << i)) != (OB_WRP_Pages0to31 << i))
    {
                                             ...
                                             SFU_BoolTypeDef *pbIsProtectionToBeApplied)
{
  ...
  else
  {
    ...
    for (uint32_t i = SFU_PROTECT_WRP_PAGE_START_1; i <= SFU_PROTECT_WRP_PAGE_END_1; i++)
    {
      psFlashOptionBytes->WRPSector |= (OB_WRP_Pages0to31 << i);
    }
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionMPU(void)
{
  ...
  if (region2_first_2kb_sub_region_disabled != 0)
  {
    /*
      ...
      */
    region2_first_2kb_sub_region_disabled = ((region2_first_2kb_sub_region_disabled + 2047) >> 11);
  }
  else
  {
    /*
  ...
  if (region1_first_16kb_sub_region_disabled <= SFU_PROTECT_MPU_MAX_NB_SUBREG)
  {
    ...
    if (((region1_first_16kb_sub_region_disabled * 16384) + (region2_first_2kb_sub_region_disabled * 2048)) <
        sbsfu_code_size)
    {
