...
#define TRAILER_SIZE (FLASH_PAGE_SIZE + (uint32_t)(TRAILER_HEADER))
...
static SFU_ErrorStatus EraseSlotIndex(uint32_t SlotNumber, uint32_t index)
{
  ...
  if ((SlotNumber == 0) && (index == 0))
  {
     ...
     *  ==> as the header (Slot #0 use case) is in the isolated area, this action should be done by Secure Engine
...
SFU_IMG_InitStatusTypeDef SFU_IMG_CoreInit(void)
{
   ...
   */
  if (((SFU_IMG_SLOT_0_REGION_HEADER_VALUE - FLASH_BASE) / (FLASH_PAGE_SIZE * 32U)) <= SFU_PROTECT_WRP_PAGE_END_1)
  {
   ...
   */
  if (((SFU_IMG_SLOT_1_REGION_BEGIN_VALUE - FLASH_BASE) / (FLASH_PAGE_SIZE * 32U)) <= SFU_PROTECT_WRP_PAGE_END_1)
  {
   ...
   */
  if (((SFU_IMG_SWAP_REGION_BEGIN_VALUE - FLASH_BASE) / (FLASH_PAGE_SIZE * 32U)) <= SFU_PROTECT_WRP_PAGE_END_1)
  {
