...
sbsfuelf=$SBSFUBootLoader"/2_Images_SBSFU/SW4STM32/STM32F769I_Discovery_2_Images_SBSFU/Debug/SBSFU.elf"
...
if [ $process == 1 ]; then
  ...
  if [ $ret == 0 ]; then
    ...
    ret=$?
    if [ $ret == 0 ]; then 
      command=$cmd" "$prepareimage" pack -k "$ecckey" -r 4 -v "$version" -i "$iv" -f "$sfu" -t "$sign" "$sfb" -o 1024"
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret == 0 ]; then
        command=$cmd" "$prepareimage" header -k  "$ecckey" -r 4 -v "$version"  -i "$iv" -f "$sfu" -t "$sign" -o 1024 "$headerbin
        $command >> $projectdir"/output.txt"
        ret=$?
        if [ $ret == 0 ]; then
