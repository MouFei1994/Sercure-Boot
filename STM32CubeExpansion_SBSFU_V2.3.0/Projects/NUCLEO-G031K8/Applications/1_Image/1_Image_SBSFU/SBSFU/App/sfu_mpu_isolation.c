...
/* Private variables ---------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define BL_EXIT_STICKY 0x1FFF1E00
...
void SFU_MPU_SVC_Handler(uint32_t *args)
{
  switch (args[0])
  {
    case SB_SYSCALL_LAUNCH_APP:
      ...
      /* Secure memory activation via bootloader service */
      launch_application(args[1], (uint32_t)exit_sticky, BL_EXIT_STICKY, MAGIC_NUMBER);
#else
