...
To fit in 32 Kbytes footprint, SFU_DEBUG_MODE compilation switch is disabled. As a consequence 
there is no print on terminal during SBSFU execution.
...
@par Hardware and Software environment
  ...
  - NUCLEO-G031K8 set-up : connect PA.15 (Arduino D2) to GND with a jumper.
  - NUCLEO-G031K8 option bytes configuration : In user configuration section, NRST_MODE should be set to 3
    to get the reset button functional.
...
7. At startup (Power On or Reset button pushed) : 
 ...
 - 'C' is displayed when SBSFU is ready for a firmware image download. To fit in 32 Kbytes footprint, 
...
Note1 : Remove the jumper between PA.15 (Arduino D2) and GND to force a local
        download at reset if an application is already installed.
