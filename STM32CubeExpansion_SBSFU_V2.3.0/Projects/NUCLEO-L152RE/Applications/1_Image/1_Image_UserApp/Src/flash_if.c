  ...
  if (e_ret_status == HAL_OK)
  {
    ...
    if (HAL_FLASH_Unlock() == HAL_OK)
    {
      ...
      do
      {
        ...
        x_erase_init.PageAddress = first_page * FLASH_PAGE_SIZE + FLASH_BASE;
...
/**
  ...
  * @param  pSource: pointer on buffer with data to write
  * @param  uLength: Length of data buffer in bytes. It has to be 4 bytes aligned.
  * @retval HAL Status.
  */
      ...
      for (i = 0U; i < uLength; i += 4U)
      {
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, (uint32_t) pDestination,  *((uint32_t *)(pdata + i))) == HAL_OK)
        {
          ...
          else
          {
            ...
            pDestination = (void *)((uint32_t)pDestination + 4U);
          }
        }
        else
        {
