...
SFU_ErrorStatus SFU_LOADER_Init(void)
{
  ...
  */
   ...
   */
  if (0 != (uint32_t)(sizeof(fw_decrypted_chunk) % (uint32_t)sizeof(SFU_LL_FLASH_write_t)))
  {
                                             ...
                                             SFU_FwImageFlashTypeDef *p_FwImageFlashData, uint32_t *puSize)
{
  ...
  *peSFU_LOADER_Status = SFU_LOADER_OK;
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    {
      *peSFU_LOADER_Status = SFU_LOADER_ERR_CRYPTO;
      e_ret_status = SFU_ERROR;
...
static SFU_ErrorStatus SFU_LOADER_VerifyFwHeader(SFU_LOADER_StatusTypeDef *peSFU_LOADER_Status, uint8_t *pBuffer)
{
  ...
  *peSFU_LOADER_Status = SFU_LOADER_ERR_CMD_AUTH_FAILED;
  ...
  if (SE_VerifyFwRawHeaderTag(&e_se_status, p_x_fw_raw_header) != SE_ERROR)
  {
    ...
    {
      /*
       * ##2 - Check length : Let's make sure the Firmware image can be written in the destination slot.
       */
...
SFU_ErrorStatus SFU_COM_YMODEM_DataPktRxCpltCallback(uint8_t *pData, uint32_t uSize)
{
  ...
  if (m_uPacketsReceived == 1)
  {
     ...
     *  ==> as the header (Slot #0 for a 1 Image use case) is in the isolated area, this action should be done by
     *      Secure Engine
     *  Thus the the erasing is split in 2 actions (HEADER + FW)
     */
    /* Clear header */
    if ((e_ret_status == SFU_SUCCESS)
        && (SFU_LL_FLASH_Erase_Size(&x_flash_info, (void *) m_uDwlAreaAddress, SFU_IMG_IMAGE_OFFSET) != SFU_SUCCESS))
    {
      e_ret_status = SFU_ERROR;
    }
    ...
    if ((e_ret_status == SFU_SUCCESS)
        && (SFU_LL_FLASH_Erase_Size(&x_flash_info, (void *)(m_uDwlAreaAddress + SFU_IMG_IMAGE_OFFSET),
                                    SFU_IMG_SLOT_DWL_REGION_SIZE - SFU_IMG_IMAGE_OFFSET) != SFU_SUCCESS))
    {
      e_ret_status = SFU_ERROR;
    }
  ...
  if (m_uDwlAreaAddress < (m_uDwlAreaStart + SFU_IMG_IMAGE_OFFSET))
  {
    ...
    if ((m_uDwlAreaAddress + uSize) < (m_uDwlAreaStart + SFU_IMG_IMAGE_OFFSET))
    {
    ...
    {
      uSize -= (m_uDwlAreaStart + SFU_IMG_IMAGE_OFFSET - m_uDwlAreaAddress);
      pData += (m_uDwlAreaStart + SFU_IMG_IMAGE_OFFSET - m_uDwlAreaAddress);
  ...
  if (uSize != 0)
  {
    fw_decrypted_chunk_size = uSize;
    if ((e_ret_status == SFU_SUCCESS)
        && (SE_Decrypt_Append(&e_se_status, pData, uSize, fw_decrypted_chunk, &fw_decrypted_chunk_size) != SE_SUCCESS))
    {
      e_ret_status = SFU_ERROR;
    }
    else if ((e_se_status != SE_OK) || (fw_decrypted_chunk_size != uSize))
    {
      e_ret_status = SFU_ERROR;
    }
    else
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    if (SFU_LL_FLASH_Write(&x_flash_info, (void *)m_uDwlAreaAddress, fw_decrypted_chunk, uSize) == SFU_SUCCESS)
    {
      m_uDwlAreaAddress += (uSize);
    }
    else
