...
/** @defgroup SFU_SECURITY_Private_Functions Private Functions
  * @{
  */
static SFU_ErrorStatus SFU_LL_SECU_CheckFlashConfiguration(FLASH_AdvOBProgramInitTypeDef *psAdvFlashOptionBytes);
static SFU_ErrorStatus SFU_LL_SECU_SetFlashConfiguration(FLASH_AdvOBProgramInitTypeDef *psAdvFlashOptionBytes,
...
SFU_ErrorStatus SFU_LL_SECU_CheckApplyStaticProtections(void)
{
  ...
  FLASH_AdvOBProgramInitTypeDef adv_flash_option_bytes;
  ...
  HAL_FLASHEx_AdvOBGetConfig(&adv_flash_option_bytes);
  ...
  if (flash_option_bytes.RDPLevel == OB_RDP_LEVEL_2)
  {
    ...
    if (SFU_LL_SECU_CheckFlashConfiguration(&adv_flash_option_bytes) != SFU_SUCCESS)
    {
  ...
  else
  {
    ...
    {
      e_ret_status = SFU_LL_SECU_SetFlashConfiguration(&adv_flash_option_bytes, &is_protection_to_be_applied);
    }
...
/**
  ...
  * @param  psAdvFlashOptionBytes: pointer to the Advanced Option Bytes structure
  * @retval SFU_ErrorStatus SFU_SUCCESS if Flash configuration is correct, SFU_ERROR otherwise.
  */
SFU_ErrorStatus SFU_LL_SECU_CheckFlashConfiguration(FLASH_AdvOBProgramInitTypeDef *psAdvFlashOptionBytes)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;

  /* Check BFB2 bit ***********************************************************/
  if (psAdvFlashOptionBytes->BootConfig == OB_BOOT_BANK1)
  {
...
/**
  ...
  * @retval SFU_ErrorStatus SFU_SUCCESS if successful, SFU_ERROR otherwise.
  */
SFU_ErrorStatus SFU_LL_SECU_SetFlashConfiguration(FLASH_AdvOBProgramInitTypeDef *psAdvFlashOptionBytes,
                                                  SFU_BoolTypeDef *pbIsProtectionToBeApplied)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;

  /* Check Flash configuration */
  if (SFU_LL_SECU_CheckFlashConfiguration(psAdvFlashOptionBytes) == SFU_SUCCESS)
  {
    e_ret_status = SFU_SUCCESS;
  }
  else
  {
    ...
    psAdvFlashOptionBytes->OptionType      = OPTIONBYTE_BOOTCONFIG;
    psAdvFlashOptionBytes->BootConfig      = OB_BOOT_BANK1;
    if (HAL_FLASHEx_AdvOBProgram(psAdvFlashOptionBytes) == HAL_OK)
    {
...
SFU_ErrorStatus SFU_LL_SECU_CheckProtectionWRP(FLASH_OBProgramInitTypeDef *psFlashOptionBytes)
{
  ...
  {
    if ((psFlashOptionBytes->WRPSector0To31  & (OB_WRP1_PAGES0TO15 << i)) != (OB_WRP1_PAGES0TO15 << i))
    {
                                             ...
                                             SFU_BoolTypeDef *pbIsProtectionToBeApplied)
{
  ...
  else
  {
    ...
    psFlashOptionBytes->WRPSector0To31   = 0x0U;
    psFlashOptionBytes->WRPSector32To63  = 0x0U;
    psFlashOptionBytes->WRPSector64To95  = 0x0U;
    psFlashOptionBytes->WRPSector96To127 = 0x0U;
    for (uint32_t i = SFU_PROTECT_WRP_PAGE_START_1; i <= SFU_PROTECT_WRP_PAGE_END_1; i++)
    {
      psFlashOptionBytes->WRPSector0To31    |= (OB_WRP1_PAGES0TO15 << i);
    }
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionMPU_UserApp(void)
{
  ...
  MPU_InitStruct.Enable               = MPU_REGION_ENABLE;
  MPU_InitStruct.Number               = APP_PROTECT_MPU_DMABBA_RGNV;
  MPU_InitStruct.BaseAddress          = APP_PROTECT_MPU_DMABBA_START;
  MPU_InitStruct.Size                 = APP_PROTECT_MPU_DMABBA_SIZE;
  MPU_InitStruct.SubRegionDisable     = APP_PROTECT_MPU_DMABBA_SREG;
  MPU_InitStruct.AccessPermission     = APP_PROTECT_MPU_DMABBA_PERM;
  MPU_InitStruct.DisableExec          = APP_PROTECT_MPU_DMABBA_EXECV;
  MPU_InitStruct.IsShareable          = MPU_ACCESS_NOT_SHAREABLE;
  MPU_InitStruct.IsBufferable         = APP_PROTECT_MPU_DMABBA_B ;
  MPU_InitStruct.IsCacheable          = APP_PROTECT_MPU_DMABBA_C;
  MPU_InitStruct.TypeExtField         = APP_PROTECT_MPU_DMABBA_TEXV;
  HAL_MPU_ConfigRegion(&MPU_InitStruct);
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionIWDG(void)
{
  ...
  /* Set counter reload value to obtain 8 sec due to slow flash access and
  slow crypto operations. IWDG TimeOut.
  IWDG counter clock Frequency = uwLsiFreq
  Set Prescaler to 128 (IWDG_PRESCALER_128)
  Timeout Period = (Reload Counter Value * 128) / uwLsiFreq
  So Set Reload Counter Value = (8 * uwLsiFreq) / 128 */
  IwdgHandle.Instance = IWDG;
  IwdgHandle.Init.Prescaler = IWDG_PRESCALER_128;
  IwdgHandle.Init.Reload = (SFU_IWDG_TIMEOUT * LSI_VALUE / 128U);
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionANTI_TAMPER(void)
{
  ...
  /* RTC_TAMPER_2 (PA0) selected, connect to CN8.1. PC13 connected to RTC_TAMPER_1 is also connected to the USER button */
