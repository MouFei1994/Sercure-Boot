...
/* Flash programming by 8 bits */
#define SHIFT_FLASH_WRITE (1U-1U)
...
ProtectionTest_t aProtectTests[] =
{
   ...
   */
  {(uint32_t)0x1FFFC000, "OBs @ 0x1FFFC000", TEST_WRITE_FLASH},
  {(uint32_t)0x1FFFC008, "OBs @ 0x1FFFC008", TEST_WRITE_FLASH},
  {(uint32_t)0x1FFFC018, "OBs @ 0x1FFFC018", TEST_WRITE_FLASH}, /* Due to MPU definition, we protect 32B instead of
                                                                   16B */
#endif /* SFU_MPU_PROTECT_ENABLE  */
  /*  Check DMA access not possible  */
  /* Check bit banding alias area for SRAM1  not accessible */
  {SRAM1_BB_BASE, "BB SRAM1_BASE  bit 0", TEST_WRITE_RAM},
  {SRAM1_BB_BASE, "BB SRAM1_BASE  bit 0", TEST_READ_RAM},
  {SRAM1_BB_BASE + 262144U * 32U - 1, "BB SRAM1_END bit 32", TEST_READ_RAM},
  {SRAM1_BB_BASE + 262144U * 32U - 1, "BB SRAM1_END bit 32", TEST_WRITE_RAM},
  /* Check bit banding alias area for DMA peripheral not accessible  */
  {PERIPH_BB_BASE + ((uint32_t)(&DMA1_Stream7->CR) - PERIPH_BASE) * 32, "BB DMA1 stream 0 CR bit 0", TEST_WRITE_RAM},
  {PERIPH_BB_BASE + ((uint32_t)(&DMA1_Stream7->FCR) - PERIPH_BASE) * 32 + 31 * 4, "BB DMA1 stream 7 FCR bit 31", TEST_WRITE_RAM},
  {PERIPH_BB_BASE + ((uint32_t)(&DMA2_Stream7->CR) - PERIPH_BASE) * 32, "BB DMA1 stream 0 CR bit 0", TEST_WRITE_RAM},
  {PERIPH_BB_BASE + ((uint32_t)(&DMA2_Stream7->FCR) - PERIPH_BASE) * 32 + 31 * 4, "BB DMA1 stream 7 FCR bit 31", TEST_WRITE_RAM},
  /* Check Flash Alias or SRAM1 alias not accessible */
  {0x0, "0x0 FLASH /SRAM1 ALIAS", TEST_WRITE_RAM},
  {0x0, "0x0 FLASH /SRAM1 ALIAS", TEST_READ_RAM},
  {0x0, "0x0 FLASH /SRAM1 ALIAS", TEST_EXECUTE},
  {0x3FFFC, "0x3FFFC FLASH /SRAM1 ALIAS", TEST_WRITE_RAM},
  {0x3FFFC, "0x3FFFC FLASH /SRAM1 ALIAS", TEST_READ_RAM},
  ...
  do
  {
    ...
    switch (aProtectTests[test_idx].type)
    {
      ...
      case TEST_WRITE_FLASH :
        HAL_FLASH_Unlock();
        HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, aProtectTests[test_idx].address, (uint8_t) pattern);
        HAL_FLASH_Lock();
