...
sbsfuelf="$SecureDir/2_Images_SBSFU/SW4STM32/STM32F413H_DISCOVERY_2_Images_SBSFU/Debug/SBSFU.elf"
...
ret=$?
if [ $ret == 0 ]; then 
  command=$cmd" "$prepareimage" pack  -k "$ecckey" -p 1 -r 44 -v "$version" -f "$bin" -t "$sign" "$sfb" -o 512" 
  $command >> $projectdir"/output.txt"
  ret=$?
  if [ $ret == 0 ]; then
    ...
    if [ $ret == 0 ]; then
      ...
      if [ $ret == 0 ] && [ -e "$ref_userapp" ]; then
        ...
        if [ $ret == 0 ]; then
          ...
          ret=$?
          if [ $ret == 0 ]; then
            command=$cmd" "$prepareimage" pack -k "$ecckey" -p 1 -r 44 -v "$version" -f "$bin" -t "$sign" -o 512 --pfw "$partialbin" --ptag "$partialsign" --poffset  "$partialoffset" "$partialsfb
            $command >> $projectdir"/output.txt"
            ret=$?
          fi
        fi
