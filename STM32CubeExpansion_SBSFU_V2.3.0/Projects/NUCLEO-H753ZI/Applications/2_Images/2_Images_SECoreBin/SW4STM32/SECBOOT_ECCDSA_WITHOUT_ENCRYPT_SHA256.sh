...
ret=$?
if [ $ret == 0 ]; then 
  command=$cmd" "$prepareimage" pack  -k "$ecckey" -p 1 -r 44 -v "$version" -f "$bin" -t "$sign" "$sfb" -o 1024" 
  $command >> $projectdir"/output.txt"
  ret=$?
  if [ $ret == 0 ]; then
    command=$cmd" "$prepareimage" header -k  "$ecckey"  -p 1 -r 44 -v "$version"  -f "$bin" -t "$sign" -o 1024 "$headerbin
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret == 0 ]; then
      ...
      if [ $ret == 0 ] && [ -e "$ref_userapp" ]; then
        ...
        if [ $ret == 0 ]; then
          ...
          ret=$?
          if [ $ret == 0 ]; then
            command=$cmd" "$prepareimage" pack -k "$ecckey" -p 1 -r 44 -v "$version" -f "$bin" -t "$sign" -o 1024 --pfw "$partialbin" --ptag "$partialsign" --poffset  "$partialoffset" "$partialsfb
            $command >> $projectdir"/output.txt"
            ret=$?
          fi
        fi
