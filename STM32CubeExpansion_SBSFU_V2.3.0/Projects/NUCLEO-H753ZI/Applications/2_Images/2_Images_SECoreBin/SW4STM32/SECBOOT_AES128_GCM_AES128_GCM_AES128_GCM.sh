...
sbsfuelf="$SecureDir/2_Images_SBSFU/SW4STM32/STM32H753ZI-Nucleo_2_Images_SBSFU/Debug/SBSFU.elf"
current_directory=`pwd`
...
if [ $ret == 0 ]; then
  ...
  ret=$?
  if [ $ret == 0 ]; then 
    command=$cmd" "$prepareimage" pack -k "$oemkey"  -r 112 -v "$version" -n "$nonce" -f "$sfu" -t "$sign" "$sfb" -o 1024"
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret == 0 ]; then
      command=$cmd" "$prepareimage" header -k  "$oemkey" -r 112 -v "$version"  -n "$nonce" -f "$sfu" -t "$sign" -o 1024 "$headerbin
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret == 0 ]; then
        ...
        if [ $ret == 0 ] && [ -e "$ref_userapp" ]; then
          ...
          command=$cmd" "$prepareimage" diff -1 "$ref_userapp" -2 "$bin" "$partialbin" -a 32 --poffset "$partialoffset
          $command >> $projectdir"/output.txt"
          ret=$?
          if [ $ret == 0 ]; then
            ...
            if [ $ret == 0 ]; then
              ...
              ret=$?
              if [ $ret == 0 ]; then
                command=$cmd" "$prepareimage" pack -k "$oemkey" -r 112 -v "$version" -i "$nonce" -f "$sfu" -t "$sign" -o 1024 --pfw "$partialsfu" --ptag "$partialsign" --poffset  "$partialoffset" "$partialsfb
                $command >> $projectdir"/output.txt"
                ret=$?
              fi
            fi
