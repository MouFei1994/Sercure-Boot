...
static void SFU_BOOT_SM_CheckUserFwStatus(void)
{
  ...
  switch (e_PendingInstallStatus)
  {
    ...
    case SFU_IMG_FWIMAGE_TO_INSTALL:
      ...
      /*
       * A new FW is present in the slot #1 and ready to be installed: to be reorganized in flash and swapped
       * with the active FW
       */
      TRACE("\r\n\t  New Fw, to be re-ordered in FLASH as expected by the swap procedure");
#else
...
static void SFU_BOOT_SM_VerifyUserFwSignature(void)
{
  ...
  if (SFU_SUCCESS == e_ret_status)
  {
    ...
    if (SFU_SUCCESS == e_ret_status)
    {
      ...
      {
#if defined(SFU_VERBOSE_DEBUG_MODE)
        TRACE("\r\n\t  Unexpected code beyond FW image in slot #0 ");
#endif /* SFU_VERBOSE_DEBUG_MODE */
      }
    }
    else
  ...
  {
#if defined(SFU_VERBOSE_DEBUG_MODE)
    TRACE("\r\n\t  Header tag verification failure ");
#endif /* SFU_VERBOSE_DEBUG_MODE */
  }
...
static void SFU_BOOT_SM_ExecuteUserFw(void)
{
  ...
  /* Configure Slot0 area in execution mode with OTFDEC (if any) : required in case of external flash */
  e_ret_status = SFU_LL_FLASH_Config_Exe(SFU_IMG_SLOT_0_REGION_BEGIN_VALUE);
  if (e_ret_status == SFU_SUCCESS)
  {
...
{
  /* First look if it is ann ECC Fault */
