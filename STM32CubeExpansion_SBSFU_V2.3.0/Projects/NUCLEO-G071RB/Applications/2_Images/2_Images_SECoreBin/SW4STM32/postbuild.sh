...
oemkey=$SBSFUBootLoader"/2_Images_SECoreBin/Binary/OEM_KEY_COMPANY1_key_AES_CBC.bin"
ecckey=$SBSFUBootLoader"/2_Images_SECoreBin/Binary/ECCKEY.txt"
sbsfuelf=$SBSFUBootLoader"/2_Images_SBSFU/SW4STM32/NUCLEO-G071RB_2_Images_SBSFU/Debug/NUCLEO-G071RB_2_Images_SBSFU.elf"
...
if [ $process == 1 ]; then
  ...
  if [ $ret == 0 ]; then
    ...
    ret=$?
    if [ $ret == 0 ]; then 
      command=$cmd" "$prepareimage" pack -k "$ecckey" -r 4 -v "$version" -i "$iv" -f "$sfu" -t "$sign" "$sfb" -o 2048"
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret == 0 ]; then
        command=$cmd" "$prepareimage" header -k  "$ecckey" -r 4 -v "$version"  -i "$iv" -f "$sfu" -t "$sign" -o 2048 "$headerbin
        $command >> $projectdir"/output.txt"
        ret=$?
        if [ $ret == 0 ]; then
