...
/* For G0, the offset image is 2048 (due to Secure Memory protection) so the chunk size must be increased too */
...
static SFU_ErrorStatus CheckMagic(uint8_t *pMagicAddr, uint8_t *pValidHeaderAddr, uint8_t *pTestHeaderAddr)
{
  ...
  /* patch signature test with 0xff on the last */
  memset(&signature_test[sizeof(signature_test) - 8], 0xff, 8);
...
{
  SFU_FLASH_StatusTypeDef flash_if_info;
  uint8_t *pCleanAddr = (uint8_t *)((uint32_t)pMagicAddr + MAGIC_LENGTH - 8);
  /* the last 8 bytes of magic are not written , they are set to 0xff after erase */
  return SFU_LL_FLASH_Write(&flash_if_info, pCleanAddr, MAGIC_NULL, 8);
}
...
static SFU_ErrorStatus WriteMagic(uint8_t *pMagicAddr, uint8_t *pHeaderValid, uint8_t *pHeaderToInstall)
{
  ...
  for (i = 0; i < MAGIC_LENGTH ; i++)
  {
    ...
    if (magic[i] != 0)
    {
      /*  don't write the last 8 bytes, for verification the last 8 bytes must be 0xff */
      e_ret_status = SFU_LL_FLASH_Write(&flash_if_info, pMagicAddr, magic, sizeof(magic) - 8);
      break;
    }
  }
  return  e_ret_status;
...
SFU_IMG_InitStatusTypeDef SFU_IMG_CoreInit(void)
{
   ...
   */
  if (((SFU_IMG_SLOT_0_REGION_HEADER_VALUE - FLASH_BASE) / FLASH_PAGE_SIZE) <= SFU_PROTECT_WRP_PAGE_END_1)
  {
