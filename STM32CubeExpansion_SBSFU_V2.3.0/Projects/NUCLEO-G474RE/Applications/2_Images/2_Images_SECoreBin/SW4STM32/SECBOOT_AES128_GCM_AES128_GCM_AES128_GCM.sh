...
if [ $ret == 0 ]; then
  ...
  ret=$?
  if [ $ret == 0 ]; then 
    command=$cmd" "$prepareimage" pack -k "$oemkey"  -r 112 -v "$version" -n "$nonce" -f "$sfu" -t "$sign" "$sfb" -o 4096"
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret == 0 ]; then
      command=$cmd" "$prepareimage" header -k  "$oemkey" -r 112 -v "$version"  -n "$nonce" -f "$sfu" -t "$sign" -o 4096 "$headerbin
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret == 0 ]; then
        ...
        if [ $ret == 0 ] && [ -e "$ref_userapp" ]; then
          ...
          if [ $ret == 0 ]; then
            ...
            if [ $ret == 0 ]; then
              ...
              ret=$?
              if [ $ret == 0 ]; then
                command=$cmd" "$prepareimage" pack -k "$oemkey" -r 112 -v "$version" -i "$nonce" -f "$sfu" -t "$sign" -o 4096 --pfw "$partialsfu" --ptag "$partialsign" --poffset  "$partialoffset" "$partialsfb
                $command >> $projectdir"/output.txt"
                ret=$?
              fi
            fi
