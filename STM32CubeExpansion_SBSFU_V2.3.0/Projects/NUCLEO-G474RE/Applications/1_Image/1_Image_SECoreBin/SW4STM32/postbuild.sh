...
if [ $ret == 0 ]; then
  ...
  ret=$?
  if [ $ret == 0 ]; then 
    command=$cmd" "$prepareimage" pack -k "$ecckey" -r 4 -v "$version" -i "$iv" -f "$sfu" -t "$sign" "$sfb" -o 4096"
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret == 0 ]; then
      command=$cmd" "$prepareimage" header -k  "$ecckey" -r 4 -v "$version"  -i "$iv" -f "$sfu" -t "$sign" -o 4096 "$headerbin
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret == 0 ]; then
