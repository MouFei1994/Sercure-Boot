...
LOADER_StatusTypeDef LOADER_InstallAtNextReset(uint8_t *pBuffer)
{
  ...
  (*(uint32_t *)LOADER_COM_REGION_RAM_START) = STANDALONE_LOADER_INSTALL_REQ;     /* Install at next reset */
  return e_ret_status;
}
...
static LOADER_StatusTypeDef LOADER_VerifyFwHeader(uint8_t *pBuffer)
{
  ...
  if (LOADER_LL_FLASH_Read((void *)&slot0_header, (void const *)REGION_SLOT_0_START, SE_FW_HEADER_TOT_LEN) != HAL_OK)
  {
  ...
  if (!memcmp(&(p_header->SFUMagic), SFUMAGIC, strlen(SFUMAGIC)))
  {
    ...
    if (e_ret_status == LOADER_OK)
    {
       ...
       */
      if (p_header->FwSize  > (SLOT_DWL_REGION_SIZE - IMAGE_OFFSET))
      {
...
static LOADER_StatusTypeDef LOADER_YMODEM_DataPktRxCpltCallback(uint8_t *pData, uint32_t uSize)
{
  ...
  if (m_uPacketsReceived == 1)
  {
    ...
    if (e_ret_status == LOADER_OK)
    {
      ...
      m_uDwlImgSize = ((SE_FwRawHeaderTypeDef *)fw_header)->FwSize + IMAGE_OFFSET;
    }
