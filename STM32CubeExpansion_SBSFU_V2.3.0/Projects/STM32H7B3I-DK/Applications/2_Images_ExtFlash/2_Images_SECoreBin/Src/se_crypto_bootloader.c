...
#if ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256) || ((SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CTR_SHA256)) )
...
SE_ErrorStatus SE_CRYPTO_Encrypt_Init(SE_FwRawHeaderTypeDef *pxSE_Metadata, int32_t SE_FwType)
{
  ...
  uint32_t fw_size;
  uint8_t *fw_tag;
#elif ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256) || ((SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CTR_SHA256)) )
...
#error "The current example does not support the selected crypto scheme."
  ...
  m_AESCBCctx.mKeySize = SE_SYMKEY_LEN;

  /* Set iv size field to IvLength*/
  m_AESCBCctx.mIvSize = SE_IV_LEN;
...
SE_ErrorStatus SE_CRYPTO_Authenticate_Metadata(SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
...
#if (SECBOOT_CRYPTO_SCHEME == SECBOOT_AES128_GCM_AES128_GCM_AES128_GCM)
  /* Variable used to store the Symmetric Key inside the protected area.
    * Please make sure this variable is placed in protected SRAM1.
    */
  static uint8_t m_aSE_FirmwareKey[SE_SYMKEY_LEN];
  ...
  /*
    * Call the DECRYPT primitive to verify the HeaderMAC:
    ...
    *   3. The HeaderMAC is the tag to be verified to authenticate the header (at the FINISH stage)
    */

  se_gcm_init.HeaderSize = ((int32_t)SE_FW_HEADER_TOT_LEN) - SE_TAG_LEN;
  se_gcm_init.PayloadSize = (int32_t) 0U;
  se_gcm_init.pNonce = (uint8_t *)pxSE_Metadata->Nonce;
  se_gcm_init.NonceSize = SE_NONCE_LEN;
  se_gcm_init.pTag = (uint8_t *) pxSE_Metadata->HeaderMAC;
