...
set "oemkey=%SBSFUBootLoader%\\2_Images_SECoreBin\\Binary\\OEM_KEY_COMPANY1_key_AES_CTR.bin"
...
set "command=%python%%prepareimage% pack -k %ecckey%  -r 36 -v %version% -n %nonce% -f %sfu% -t %sign% %sfb% -o 1024 >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set "command=%python%%prepareimage% header -k  %ecckey% -r 36 -v %version%  -n %nonce% -f %sfu% -t %sign% -o 1024 %headerbin% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
...
set "command=%python%%prepareimage% extract -d "__ICFEDIT_region_SLOT_0_header__=" %mapping% > %projectdir%\header.txt"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
...
set "command=%python%%prepareimage% pack -k %ecckey%  -r 36 -v %version% -n %nonce% -f %sfu% -t %sign% -o 1024 --pfw %partialsfu% --ptag %partialsign% --poffset  %partialoffset% %partialsfb%>> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
