...
#elif (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CTR_SHA256)
/* AES-CTR encryption (decrypted with OTFDEC) and SHA256 for FW tag */
#define SE_NONCE_LEN            ((int32_t) 8)   /*!< Secure Engine Nonce Length (Bytes)*/
#define SE_TAG_LEN              ((int32_t) 32)  /*!< Secure Engine Tag Length (Bytes): SHA-256 for the FW tag */
#else
...
#elif (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CTR_SHA256)
#define SE_FW_HEADER_METADATA_LEN  (SE_FW_HEADER_TOT_LEN-(36+SE_MAC_LEN))
#endif /* SECBOOT_CRYPTO_SCHEME */
/**
  * @}
  */
...
typedef struct
{
  ...
  uint8_t  PartialFwTag[SE_TAG_LEN];/*!< Partial firmware Tag */
  uint8_t  Nonce[SE_NONCE_LEN];    /*!< Nonce used to encrypt firmware*/
  uint8_t  Reserved[36];           /*!< Reserved for future use: 36 extra bytes to have a header size of 192 bytes */
  uint8_t  HeaderMAC[SE_MAC_LEN];  /*!< MAC of the full header message */
} SE_FwRawHeaderTypeDef;
