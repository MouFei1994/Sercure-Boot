...
static uint32_t LOADER_LL_FLASH_INT_GetBankAddr(uint32_t bank);
  ...
  if (e_ret_status == HAL_OK)
  {
    ...
    if (e_ret_status == HAL_OK)
    {
      ...
      do
      {
        ...
        bank_number = LOADER_LL_FLASH_INT_GetBank(start);
        if (LOADER_LL_FLASH_INT_GetBank(start + Length - 1U) == bank_number)
        {
        ...
        {
          uint32_t startbank2 = LOADER_LL_FLASH_INT_GetBankAddr(FLASH_BANK_2);
          nb_sectors = LOADER_LL_FLASH_INT_GetSector(startbank2 - 1U) - first_sector + 1U;
...
HAL_StatusTypeDef LOADER_LL_FLASH_INT_Write(void  *pDestination, const void *pSource, uint32_t Length)
{
  ...
  if (e_ret_status == HAL_OK)
  {
    ...
    if (e_ret_status == HAL_OK)
    {
      ...
      {
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, (uint32_t)pDestination, (uint64_t)(pdata + i)) == HAL_OK)
        {
          /* Check the written value */
          if (memcmp((void *)pDestination, (void *)(pdata + i), sizeof(LOADER_LL_FLASH_write_t)) != 0U)
          {
