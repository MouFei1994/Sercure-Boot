...
/* For MCU based on Cortex M7 we need the UserApp start offset to be 1024 bytes (Cortex-M7 init.vector alignment
   constraint) so the chunk size must be increased too */
#define SFU_IMG_CHUNK_SIZE  (1024U)
...
{
  SFU_FLASH_StatusTypeDef flash_if_info;
  /* erase area where Trailer Magic is located */
  return SFU_LL_FLASH_Erase_Size(&flash_if_info, pMagicAddr, MAGIC_LENGTH);
}
...
static SFU_ErrorStatus EraseSlotIndex(uint32_t SlotNumber, uint32_t index)
{
  ...
  pbuffer = (uint8_t *) SlotBeginAddress[SlotNumber];
  pbuffer = pbuffer + (SFU_IMG_SWAP_REGION_SIZE * index);

  if ((SlotNumber == 0) && (index == 0))
  {
    /*
     *  As the header is not contiguous with FW, this action is split in 2 actions (HEADER + remaining part)
     */
    e_ret_status = SFU_LL_FLASH_Erase_Size(&flash_if_status, (uint8_t *) SlotHeaderAddress[SlotNumber],
                                           SFU_IMG_IMAGE_OFFSET) ;
    if (e_ret_status == SFU_SUCCESS)
    {
      e_ret_status = SFU_LL_FLASH_Erase_Size(&flash_if_status, (uint8_t *) SlotBeginAddress[SlotNumber] +
                                             SFU_IMG_IMAGE_OFFSET, SFU_IMG_SWAP_REGION_SIZE - SFU_IMG_IMAGE_OFFSET);
    }
  }
  else
  {
...
static SFU_ErrorStatus SwapFirmwareImages(void)
{
  ...
  while (index_slot0 >= 0)
  {
    ...
    /* If CPY_TO_SLOT1(i) is still virgin, then swap the block from slot #0 to
       slot #1 */
    ...
    /* If CPY_TO_SLOT0(i) is still virgin, then swap the block from slot #1 to
    slot #0 */
           ...
           (e_se_status == SE_OK))
    {
      ...
      if (pass_index == 0)
      {
        ...
        pass_index = (((SFU_IMG_IMAGE_OFFSET + (pFwImageHeader->PartialFwOffset % SFU_IMG_SWAP_REGION_SIZE))
                       % SFU_IMG_SWAP_REGION_SIZE) / sizeof(fw_decrypted_chunk));
      }
      else
      {
...
SFU_IMG_InitStatusTypeDef SFU_IMG_CoreInit(void)
{
   ...
   */
  if (((SFU_IMG_SLOT_0_REGION_HEADER_VALUE - FLASH_BASE) / FLASH_SECTOR_SIZE) <= SFU_PROTECT_WRP_PAGE_END_1)
  {
   ...
   */
  if (((SFU_IMG_SLOT_1_REGION_BEGIN_VALUE - FLASH_BASE) / FLASH_SECTOR_SIZE) <= SFU_PROTECT_WRP_PAGE_END_1)
  {
   ...
   */
  if (((SFU_IMG_SWAP_REGION_BEGIN_VALUE - FLASH_BASE) / FLASH_SECTOR_SIZE) <= SFU_PROTECT_WRP_PAGE_END_1)
  {
...
SFU_ErrorStatus SFU_IMG_WriteHeaderValidated(uint8_t *pHeader)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    /*  write in flash  */
    e_ret_status = SFU_LL_FLASH_Write(&flash_if_status, SFU_IMG_SLOT_0_REGION_HEADER, FWInfoInput, sizeof(FWInfoInput));
    STATUS_FWIMG(e_ret_status == SFU_ERROR, SFU_IMG_FLASH_WRITE_FAILED);
  }
  return e_ret_status;
}
...
SFU_ErrorStatus  SFU_IMG_CheckTrailerValid(void)
{
  ...
  e_ret_status = SFU_LL_FLASH_Read(FWInfoValid, SFU_IMG_SLOT_0_REGION_HEADER, sizeof(FWInfoValid));
  STATUS_FWIMG(e_ret_status == SFU_ERROR, SFU_IMG_FLASH_READ_FAILED);
...
SFU_ErrorStatus SFU_IMG_FirmwareToInstall(void)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      /*
       * The image to install must not be already validated.
       */
      e_ret_status = CheckHeaderValidated(fw_header_slot);
...
/**
  ...
  * @note   relies on following global in ram, already filled by the check
  *         fw_header_to_test,fw_image_header_validated, fw_image_header_to_test
  * @retval SFU_SUCCESS if successful,SFU_ERROR error otherwise.
  */
